...
def validate_task(
    ...
    policies = business.get("policies", {}) if isinstance(business.get("policies"), dict) else {}
    ...
    if mode == "production" and policies.get("require_test_mode") is True and approval.get("require_test_approval") is not True:
    ...
    if not isinstance(validation.get("duplicate_key"), str) or not validation["duplicate_key"].strip():
